...
def gradient_method(standardized_x, y):
    ...
    # パラメーターの初期値を使用した目的関数の値をerror_beforeに代入
    ...
    x_count = np.arange(len(differences)) # 記録された差分の範囲で等差数列を生成
    plt.plot(x_count, differences)        # 差分の変化をプロット
    plt.grid(True)
    plt.show()                            # グラフを表示
...
if __name__ == '__main__':
    ...
    #print(f(standardize(300), parameter_0, parameter_1))
